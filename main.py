...
'''common arguments'''
parser.add_argument('--weight', type=int, default=0.0001)
...
def _train(train_data, model, train_gloss_dict, optim, schedule, criterion, gloss_bsz=-1, max_grad_norm=1.0, multigpu=False, silent=False, train_steps=-1):
	model.train()
	...
	for i, (context_ids, context_attn_mask, context_output_mask, example_keys, _, labels) in train_data:
		...
		for j, (key, label) in enumerate(zip(example_keys, labels)):
			...
			glosses_output  = model.gloss_forward(gloss_ids, gloss_attn_mask, None)
			...
			c_x_hat, c_sampled_z, c_mu1, c_lv1, c_mu2, c_lv2 = model.context_vae_forward(output)
			s_x_hat, s_sampled_z, s_mu1, s_lv1, s_mu2, s_lv2 = model.gloss_vae_forward(glosses_output)



			result_1 = torch.mm(output, glosses_output.transpose(0,1))
			result_2 = torch.mm(c_x_hat, s_x_hat.transpose(0,1))
			result   = result_1 * 2 + result_2
			...
			loss += criterion[key](result, label_tensor) + model.gloss_vae_loss(s_mu1, s_lv1, s_mu2, s_lv2, weight=args.weight) + model.context_vae_loss(c_mu1, c_lv1, c_mu2, c_lv2, weight=args.weight)
			


			gloss_sz += glosses_output.size(-1)
...
def _eval(eval_data, model, gloss_dict, multigpu=False):
	...
	for context_ids, context_attn_mask, context_output_mask, example_keys, insts, _ in eval_data:
		with torch.no_grad(): 
			...
			for j, (output, key, inst) in enumerate(zip(context_output.split(1,dim=0), example_keys, insts)):
				...
				if multigpu:
					gloss_ids       = gloss_ids.to(context_output)
					gloss_attn_mask = gloss_attn_mask.to(context_output)
				else:
					gloss_ids       = gloss_ids.cuda()
					gloss_attn_mask = gloss_attn_mask.cuda()
...
def train_model(args):
	...
	if args.multigpu: 
		...
		model.map    				 = model.map.to(context_device)
		model.vae                    = model.vae.to(context_device)
	...
	for epoch in range(1, epochs+1):
		...
		model, optimizer, schedule = _train(train_data, model, train_gloss_dict, optimizer, schedule, criterion, gloss_bsz=args.gloss_bsz, max_grad_norm=args.grad_norm, silent=args.silent, multigpu=args.multigpu, train_steps=train_steps)
